...
using WarehouseManger.Infrastructure.Helpers;
...
using WarehouseManger.Shared.Constants.User;
...
namespace WarehouseManger.Infrastructure
{
    public class DatabaseSeeder : IDatabaseSeeder
    {
        ...
        private void AddDoctorDetails()
        {
            ...
            {
                //Check if Role Exists
                var basicRole = new BlazorHeroRole(RoleConstants.BasicRole, _localizer["Administrator role with full permissions"]);
                var basicRoleInDb = await _roleManager.FindByNameAsync(RoleConstants.AdministratorRole);
                if (basicRoleInDb == null)
                {
                    await _roleManager.CreateAsync(basicRole);
                    basicRoleInDb = await _roleManager.FindByNameAsync(RoleConstants.AdministratorRole);
                    _logger.LogInformation(_localizer["Seeded Administrator Role."]);
                }

               var doctorsList =  await _db.Set<DoctorDetails>().ToListAsync();
                if(doctorsList == null || doctorsList.Count == 0)
                {
                    var doctorDetails = new DoctorDetails
                    {
                        DoctorId = Guid.NewGuid(),
                        DoctorName = "Atul Gupta",
                        DateOfBirth = DateTime.Now.AddYears(-40),
                        Department = "General Physician",
                        CreatedOn = DateTime.Now

                    };
                    await _db.Set<DoctorDetails>().AddAsync(doctorDetails);
                }
                //Check if User Exists
